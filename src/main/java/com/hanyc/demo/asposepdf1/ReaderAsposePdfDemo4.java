 ...
 * 终版!!!!
 */
public class ReaderAsposePdfDemo4 {
    ...
    public static void main(String[] args) {
        ...
        for (int pageIndex = 1; pageIndex <= pdfDocument.getPages().size(); pageIndex++) {
            ...
            for (int paraNum = 0; paraNum < targetPage.getParagraphs().getCount(); paraNum++) {
                BaseParagraph item = targetPage.getParagraphs().get_Item(paraNum);
                if (item instanceof Table) {
                    Rows rows = ((Table) item).getRows();
                    for (int rowNum = 0; rowNum < rows.getCount(); rowNum++) {
                        Row row = rows.get_Item(rowNum);
                        Cells cells = row.getCells();
                        for (int cellNum = 0; cellNum < cells.getCount(); cellNum++) {
                            Cell cell = cells.get_Item(cellNum);
                            Paragraphs cellParagraphs = cell.getParagraphs();
                            for (int cellParagraphNum = 0; cellParagraphNum < cellParagraphs.getCount(); cellParagraphNum++) {
                                BaseParagraph cellParagraphsItem1 = cellParagraphs.get_Item(cellParagraphNum);
//                                TableParagraph
//                                TextParagraph cellParagraphsItem = (TextParagraph) cellParagraphsItem1;
//                                System.out.println(cellParagraphsItem.getText());
                            }
                        }
